...
interface residues within 4.5 A, and emits:
  - data/targets/<name>_target.pdb
  - data/targets/<name>_complex.pdb (when a partner exists)
...
# Distal / tip-ish atoms for RFD3 select_hotspots (matches foundry binder docs style).
...
# Literature-known interface residues used as soft cross-checks (auth numbering).
LITERATURE_HOTSPOTS: dict[str, list[int]] = {
    "kras": [37, 38, 40],  # switch I effector face (E37/D38/Y40)
    "stat3": [609, 611, 613],  # SH2 pTyr pocket
    "beta_catenin": [312, 435],  # charged buttons in TCF groove
}

# Official RFD3 PD-L1 example (foundry protein_binder_design.md) — use verbatim.
...
def fetch_assembly_pdb(pdb_id: str, assembly: int = 1) -> Path:
    """Download biological assembly and normalize to a ``.pdb`` path gemmi can read."""
...
def gemmi_read(path: Path) -> gemmi.Structure:
    # Biological assemblies are often named *.pdb1; gemmi needs a known suffix.
...
def crop_structure(
    ...
    """Return a new Structure keeping only selected polymer residues."""
    ...
    for chain_id in keep_chains:
        ...
        for res in src_chain:
            if strip_hetero and res.het_flag == "H":
                continue
            # Keep polymer residues only (standard AA + common modified)
            if res.entity_type not in (gemmi.EntityType.Polymer, gemmi.EntityType.Unknown):
                # Some PDBs mark polymers as Unknown; fall back to residue name check
...
def count_interface_contacts(
    ...
    """Rank target residues by heavy-atom contacts to partner within cutoff."""
    tgt = st[0][target_chain]
...
def pick_hotspots(
    ...
    """Return select_hotspots dict and notes about literature overlap."""
    notes: list[str] = []
    if forced:
...
def hotspot_covering_segment(
    ...
    """Largest continuous observed segment that covers as many hotspots as possible."""
    segs = continuous_segments(nums)
    if not segs:
        return None
    best = None
    best_score = (-1, -1)  # (n_hotspots_covered, length)
    for lo, hi in segs:
...
def build_contig(binder_lo: int, binder_hi: int, target_chain: str, res_lo: int, res_hi: int) -> str:
    # RFD3 binder pattern: "<binder_len>,/0,<target_chain><lo>-<hi>"
...
def prepare_target(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    st = gemmi_read(raw)
    # Biological assemblies can rename chains (A, A-2, etc.) — normalize first model chain names.
    chain_names = [c.name for c in st[0]]
    log(f"  chains in file: {chain_names}")

    keep = list(spec["keep_chains"])
    # Map logical names if assembly remapped
    if spec.get("chain_remap"):
        keep = [spec["chain_remap"].get(c, c) for c in keep]
    ...
    if spec.get("chain_remap"):
        target_chain = spec["chain_remap"].get(target_chain, target_chain)

    partner_chain = spec.get("partner_chain")
    if partner_chain and spec.get("chain_remap"):
        partner_chain = spec["chain_remap"].get(partner_chain, partner_chain)
    ...
    # Contigs must not span missing residues — crop to continuous segment covering hotspots.
    ...
    if (seg_lo, seg_hi) != (lo, hi) or len(nums) != (hi - lo + 1):
        ...
        write_gemmi_pdb(target_only, target_path)
        # Keep partner in complex but also trim target chain
        if partner_chain:
    ...
    contigs = {
        f"len_{a}_{b}": build_contig(a, b, target_chain, lo, hi) for a, b in binder_bins
    }
...
def target_specs() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "target_chain": "A",
            "partner_chain": "B",
            "residue_ranges": None,  # keep observed MYC/MAX; DNA stripped as hetero/non-polymer
        ...
        },
        {
            ...
            "pdb_id": "1BG1",
            "assembly": 1,
            "keep_chains": ["A", "B"],  # may be remapped in assembly
            "target_chain": "A",
            "partner_chain": "B",
            "residue_ranges": None,
            ...
            # Target the SH2 pTyr pocket (binder epitope), not only the reciprocal C-term contacts
            "forced_hotspots": {
                "A609": "NH1,NH2",
                "A611": "OG",
                "A613": "OG",
        ...
        },
        {
            ...
            "target_chain": "A",
            "partner_chain": "B",
            # ARM repeats covering TCF contact; trims disordered termini for token budget
            "residue_ranges": {"A": (134, 664), "B": None},
            "why_undruggable": "Extended shallow armadillo groove used by TCF/LEF/APC/cadherin",
            "caveats": ["Cropped ARM repeats 134-664; hotspots from beta-catenin–hTcf-4 contacts"],
            "n_hotspots": 3,
        },
    ]
...
def merge_multimodel_dimer(raw: Path, out_path: Path) -> Path | None:
    """If assembly stores the dimer as two models of chain A, merge into A+B."""
    st = gemmi_read(raw)
    if len(st) < 2:
        return None
    polymers_per_model = []
...
def rename_chains_in_place(raw: Path, remap: dict[str, str], out_path: Path) -> Path:
    st = gemmi_read(raw)
    # Build new structure with remapped names; drop other chains
    new = gemmi.Structure()
    new.name = st.name
    new.cell = st.cell
    new.spacegroup_hm = st.spacegroup_hm
    ...
    for new_name in sorted(inverse.keys()):
        old_name = inverse[new_name]
        ch = gemmi.Chain(new_name)
        for res in st[0][old_name]:
            ch.add_residue(res)
        model.add_chain(ch)
    new.add_model(model)
    write_gemmi_pdb(new, out_path)
...
def main() -> int:
    ...
    # STAT3: biological assembly stores the SH2 dimer as two models — merge to A/B.
    for s in specs:
        if s["name"] == "stat3":
            ...
            s["target_chain"] = "A"
            s["partner_chain"] = "B"
            # Focus on SH2 domain region around pTyr-705 pocket for token budget
            s["residue_ranges"] = {"A": (500, 716), "B": (500, 716)}

    registry: list[dict[str, Any]] = []
    for spec in specs:
        registry.append(prepare_target(spec))
