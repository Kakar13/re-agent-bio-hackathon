...
Uses Baker-lab RFD3 binder settings (hotspots ori, is_non_loopy, low-temp
step_scale=3 / gamma_0=0.2). Chunked + resumable: each call produces
n_bins * diffusion_batch_size designs and records a manifest row.
...
def chunk_done(target: str, chunk_idx: int) -> bool:
    marker = chunk_dir(target, chunk_idx) / "done.json"
...
def save_designs(
    ...
    """Write PDBs; return per-design records."""
    ...
    for bundle in out.designed_structures:
        for structure_wrap in bundle:
            ...
            # Also keep a copy under the chunk for provenance
...
def run_target(
    ...
    *,
    ...
    # Trim extras if we overshot (keep first n)
...
def main() -> int:
    ...
    if args.target == "all":
        ...
        targets = [target_by_name(registry, args.target)]
